...
const ReportListing = () => {
  ...
  const getReportList = async () => {
    try {
      const response = await axios.get(`https://tolet-server-mu.vercel.app/reports`);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://tolet-server-mu.vercel.app/reportLists/${id}`);
            ...
            {reportListData.map((flat, index) => (
              ...
              >
                <td className="py-4 px-4 flex justify-start">
                  <img
                    src={`https://tolet-server-mu.vercel.app/image/${flat?.flatWishList?.images}`}
                    alt="Flat"
