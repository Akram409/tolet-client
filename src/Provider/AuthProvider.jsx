...
const AuthProvider = ({ children }) => {
  ...
  const verifyToken = async () => {
    try {
      ...
      if (token) {
        const response = await axios.get("https://tolet-server-mu.vercel.app/verifyToken", {
          headers: {
  ...
  const login = async (email, password) => {
    try {
      const response = await axios.post("https://tolet-server-mu.vercel.app/login", {
        email,
        password,
      });
  ...
  const fetchUserData = async (email) => {
    try {
      const response = await axios.get(`https://tolet-server-mu.vercel.app/user/${email}`);
